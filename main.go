...
import (
	"bytes"
	"context"
	"flag" // NEW: Added for flag parsing
	"fmt"
	"hash/fnv"
	"io"
	"net/http"
...
type Queue struct {
	...
	elements    []string
	mu          sync.Mutex
	useDFS      bool // NEW: Added DFS flag
}
...
func (q *Queue) dequeue() string {
	...
	var url string
	if q.useDFS {
		// DFS: Remove last element (LIFO)
		url = q.elements[len(q.elements)-1]
		q.elements = q.elements[:len(q.elements)-1]
	} else {
		// BFS: Remove first element (FIFO)
		url = q.elements[0]
...
func main() {
	// NEW: Parse DFS flag
	useDFS := flag.Bool("dfs", false, "Use DFS algorithm")
	flag.Parse()
	...
	// NEW: Initialize queue with DFS flag
	queue := Queue{
		useDFS:      *useDFS, // Set DFS mode here
		totalQueued: 0,
		number:      0,
